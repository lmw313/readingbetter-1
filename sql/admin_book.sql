...
-- 도서 관리 수정
update book set title='햄스터 시리즈 01 개정판', RECOMMEND=3, cover='http://cfile8.uf.tistory.com/image/272EFE3753B673201D0EFA', PUBLISHER_NO=(select no from publisher where title='햄스터 출판사'), AUTHOR_NO=(select no from author where name='햄스터 마니아') where no=165;
select * from book order by no asc;